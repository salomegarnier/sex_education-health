...
life_expectancy <- read_excel("Raw_data/life_expectancy.xlsx") %>%
  ...
  slice(1:774) %>%
  pivot_wider(names_from = 'Series Name', values_from = '2020 [YR2020]', id_cols = 'Country Name', values_fn = as.numeric) %>%
...
health_outcomes <- full_join(hiv_to_join, adolescent_preg, by = "country") %>%
  full_join(., contraception, by = "country") %>%
  left_join(., young_people_hiv, by = "country") %>%
  full_join(., maternal_mortality, by = "country") %>%
